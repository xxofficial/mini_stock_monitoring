...
- **个性化设置**：支持紧凑模式、无背景的极简模式、行情刷新模式、刷新间隔、窗口位置和全局快捷键配置。
...
全局快捷键可在设置中重新录制、停用或恢复默认值。窗口高度会随自选数量调整，超过 7 行后自动滚动；开启“紧凑显示”可进一步缩小占用空间。极简模式会自动置顶悬浮，仅保留股票名、股价和涨跌幅文字，可按设置选择是否在鼠标悬停时临时显示对应分时图；分时图会根据当前显示器工作区自动选择右、左、下或上方，避免超出屏幕。极简窗口仍可拖动，开始拖动时会自动收起分时图。可通过托盘右键菜单“退出极简模式”恢复完整界面与原有置顶设置。
...
其中包含自选列表、透明度、置顶状态、紧凑模式、极简模式、行情模式、刷新间隔、窗口位置和显示 / 隐藏快捷键。保存时会先写入临时文件再替换原文件，避免意外中断损坏配置；若发现配置格式异常，程序会将原文件备份为 `settings.invalid-*.json`。
